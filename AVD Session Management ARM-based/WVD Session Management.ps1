...
{
    $dgvHosts.Rows.Clear()
    $HostpoolObject = Get-AzWvdHostPool | Where name -eq $Hostpool
    $UserSessions = Get-AzWvdUserSession -HostPoolName $HostpoolObject.Name -ResourceGroupName $HostpoolObject.Id.Split("/")[4]
    $SessionHosts = Get-AzWvdSessionHost -HostPoolName $HostpoolObject.Name -ResourceGroupName $HostpoolObject.Id.Split("/")[4]
    $HostpoolProperties = Get-AzWvdHostPool -HostPoolName $HostpoolObject.Name -ResourceGroupName $HostpoolObject.Id.Split("/")[4]
...
$TenantNode = $tvPools.Nodes.Add("Hostpools")

$Hostpools = Get-AzWvdHostPool | foreach{
...
$Main.Name = "Main"
$Main.Text = "WVD Session Management [connected to Resource Group $TenantID]"
